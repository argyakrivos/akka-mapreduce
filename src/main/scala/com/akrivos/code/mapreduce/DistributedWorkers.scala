...
class Master extends Actor with ActorLogging {
  import com.akrivos.code.mapreduce.MasterWorkerProtocol._
  ...
  override def receive: Receive = {
    ...
    case work =>
      log.debug("Queueing {}", work.toString)
...
abstract class Worker(masterLocation: ActorPath) extends Actor with ActorLogging {
  ...
  def working(work: Any): Receive = {
    ...
    case WorkComplete(result) =>
      log.debug("Work is complete. Result {}.", result.toString)
  ...
  def idle: Receive = {
    ...
    case WorkToBeDone(work) =>
      log.debug("Got work {}", work.toString)
      doWork(sender(), work)
